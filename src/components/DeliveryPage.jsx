import React, { useState,useReducer, useEffect ,useContext} from "react";
import DataContext from "../context/DataContext";
import {fetchMenu} from "../functions/fetching.js"
import { Navigate, useNavigate } from "react-router-dom";

function DeliveryPage(props){ 
    const navigate=useNavigate();
    const { Data,setView,foodData,setFoodData,setURL,backendURL}=useContext(DataContext)

    setURL("/");

    async function View(e){
        e.preventDefault()
        const data=await fetchMenu(backendURL);
         console.log(data)
        setFoodData(data)
        navigate("/food")

        return null
    }
  
if (Data=== undefined || Data=== null){return(<></>)}
else{
    return(
       
<div className=" text-white grid grid-cols-4 gap-4">
            {/* mapping will be done here  */}
       {Data.map((item)=>(

<div key={item.id} className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
    <a href="#">
        <img className=" w-96 h-80  p-8 rounded-lg" src={item.featured_image} alt="product image" />
    </a>
    <div className="px-5 pb-5">
        <a href="#">
            <h5 className="text-xl font-semibold tracking-tight  dark:text-white">{item.name}</h5>
        </a>
        <div className="flex items-center mt-2.5 mb-5">
        <p className="mr-auto">{item.cuisines}</p>
            <span className="ml-auto bg-blue-100 flex text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800">{item.user_rating.aggregate_rating}
            <svg className="pl-0.5 w-4 h-4 text-yellow-300 mr-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"/>
            </svg>
            </span>
            
        </div>
        
        <div className="flex ">
        <div className=" leading-none flex items-center mr-auto"><p className="p-1">Just</p>
            <span className="text-3xl font-bold  dark:text-white">Rs{item.average_cost_for_two}</span><p className="p-1">for 2 people</p>
            
        </div >
        <a href="#" onClick={View} className="ml-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">View</a>
        </div>
</div>
</div>))}
</div>

        
    )}
}

export default DeliveryPage